...
# m, n = 1e-2, 10   ###### 最佳种群数150
# m, n = 1, 1         ###### 最佳种群数150
m, n = 10, 1e-2   ###### 最佳种群数180
...
def test_weight_combinations_enhanced():
    ...
    for idx, (weight_m, weight_n) in enumerate(weight_combinations, 1):
        ...
        for retry in range(max_retries):
            ...
            print(f"  NPDOA完成，最佳成本: {npdoa_cost:.2f}")
...
def run_npdoa_sac(runs=1, total_steps=20_000):
    ...
    print(f"NPDOA阶段完成，最佳成本: {npdoa_cost:.2f}")
    ...
    plt.title('SAC Training Process (with Improved NPDOA initialization)')
    plt.grid(True)
...
if __name__ == "__main__":
    final_best_cost = run_npdoa_sac(runs=1)
