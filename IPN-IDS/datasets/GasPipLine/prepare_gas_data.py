...
def split_meta_datasets(all_data, params):
    ...
    meta_train_df = meta_train_df.groupby("Label", group_keys=False).apply(
        lambda x: x.sample(min_sample_count_per_train_class))
    meta_test_df = meta_test_df.groupby("Label", group_keys=False).apply(
        lambda x: x.sample(min_sample_count_per_test_class))

    meta_val_df_from_train = meta_train_df.groupby("Label", group_keys=False).apply(
        lambda x: x.sample(validation_sample_per_class)).copy()
    meta_train_df = meta_train_df.drop(meta_val_df_from_train.index)
    meta_val_df_from_test = meta_test_df.groupby("Label", group_keys=False).apply(
        lambda x: x.sample(validation_sample_per_class)).copy()
...
def pre_process_dataset(params):
    ...
    all_data = load_datasets(params["gas_files_list"])
    ...
    logging.info("\n{}".format(label_perc))
    all_data.iloc[:, :26] = normalize_dataset(all_data.iloc[:, :26])
    print(all_data)
    all_data.replace(np.nan, 0, inplace=True)
    return all_data


def prepare_gas_datasets(params):
    all_data = pre_process_dataset(params)

    x_meta_train, y_meta_train, x_meta_test, y_meta_test, x_meta_val, y_meta_val, remaining_df = split_meta_datasets(
        all_data, params)
    ...
    logging.info("{} Labels used int meta train = \n{}".format(len(y_meta_train.unique()),
                                                               ", ".join([str(x) for x in y_meta_train.unique()])))
    logging.info("{} Labels used int meta test = \n{}".format(len(y_meta_test.unique()),
                                                              ", ".join([str(x) for x in y_meta_test.unique()])))
    logging.info("{} Labels used int meta val = \n{}".format(len(y_meta_val.unique()),
                                                             ", ".join([str(x) for x in y_meta_val.unique()])))


def parse_configuration():
    parser = argparse.ArgumentParser()
    parser.add_argument("--hdf_key", type=str, default="gas")
    parser.add_argument("--output_dir_prefix", type=str, default="gas_prepared")
    parser.add_argument("--gas_datasets_dir", type=str, default="gas")
    parser.add_argument("--n_train", type=int, default=6)
    parser.add_argument("--k_shot", type=int, default=10)
    parser.add_argument("--q_train", type=int, default=450)
    parser.add_argument("--n_test", type=int, default=2)
    parser.add_argument("--q_test", type=int, default=450)
    parser.add_argument("--q_val", type=int, default=100)
    ...
    params = {
        ...
        "gas_files_list": [
            config.gas_datasets_dir + "/gas1.csv"
        ]
    }
...
if __name__ == "__main__":
    ...
    prepare_gas_datasets(parameters)
