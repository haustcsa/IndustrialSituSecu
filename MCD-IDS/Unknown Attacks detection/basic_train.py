...
def train(model, train_x, train_y, config, writer):
    ...
    scheduler = optim.lr_scheduler.StepLR(optimizer, 1, gamma=config.learning_rate_decay, last_epoch=-1)#创建一个学习率调度器，用于在每个 epoch 后按指定的衰减率调整学习率。
...
def validate(model, x_val, y_val, config, writer):
    model.eval()
    sample_size = x_val.shape[0]#获取验证集的大小（样本数量）
    model.n_way = len(y_val.unique())#模型参数n_way为验证集中类别的数量
