...
class Pipeline:
    ...
    def dictionary_and_embedding(self, vector_size):
        self.size = vector_size  # 将 vector_size 保存到 self.size
        trees = self.train
        self.w2v_path = 'subtrees/' + args.input + '/node_w2v_' + str(vector_size)
        ...
        if not os.path.exists(self.w2v_path):
            ...
            w2v = Word2Vec(corpus, vector_size=vector_size, workers=96, sg=1, min_count=3)
            print('word2vec : ', w2v)
    ...
    def run(self, dataset):
        ...
        self.dictionary_and_embedding(vector_size=128)
