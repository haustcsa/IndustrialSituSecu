...
#第二次修改，为源代码可跑通 与上面相比，添加了进度条以及时间显示，其他一样，上面也可以跑通
import argparse
import os
import pandas as pd
import torch
...
if __name__ == '__main__':
    ...
    # training procedure with tqdm for progress bar
    for epoch in range(EPOCHS):
        start_time = time.time()
        total_acc = 0.0
        total_loss = 0.0
        ...
        # Use tqdm to display the progress bar for batches
        epoch_progress = tqdm(range(0, len(train_data), BATCH_SIZE), desc=f"Epoch {epoch + 1}/{EPOCHS}", ncols=100)
        ...
        for i in epoch_progress:
            ...
            epoch_progress.set_postfix(loss=total_loss / total, acc=total_acc.item() / total)
        ...
        # Print epoch summary
        print('-' * 89)
        print(f'| end of epoch {epoch + 1:3d} / {EPOCHS:3d} | time: {(time.time() - start_time):5.2f}s | '
              f'train loss {total_loss / total:5.2f} | train acc {total_acc.item() / total:5.2f} | '
        ...
        if val_data is not None:
            ...
            val_progress = tqdm(range(0, len(val_data), BATCH_SIZE), desc=f"Validation {epoch + 1}/{EPOCHS}", ncols=100)
            ...
            for i in val_progress:
                ...
                # Update tqdm progress bar description
                val_progress.set_postfix(loss=total_loss / total, acc=total_acc.item() / total)
