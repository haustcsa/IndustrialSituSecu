...
plt.style.use([r'D:\PythonCode\SimpleNet-master\venv\Lib\site-packages\scienceplots\styles\science.mplstyle', r'D:\PythonCode\SimpleNet-master\venv\Lib\site-packages\scienceplots\styles\journals\ieee.mplstyle'])
...
if len(argv) > 1:
    env = argv[1]  # 如果传入了环境参数，使用它
    print('ssssssssss', env)  # 这里的 print 需要与 env = argv[1] 对齐
else:
    env = 'default_environment'  # 如果没有传入环境参数，使用默认值
    print('sdsadasdasdasdasd', env)
    print("No environment argument provided, using default environment:", env)


print('envenvenvenvenv===',env)
option = 0
sla_baseline = 'GOBI'
rot = 25
...
# Models = ['SGATGAN', 'CMODLB', 'PCFT', 'ECLB', 'DFTM', 'GOBI']
Models = ['SGATGAN']
...
for model in load_models:
	...
	try:
		...
		for file in os.listdir(PATH+model2):
			...
			#if fnmatch.fnmatch(file, '*.ckpt'):
				print('444444444444444444')
				print(file)
				with open(PATH + model2 + '/' + file, 'rb') as handle:
					print('ssssssssssssdd5555')
...
# # 定义指标
# yLabelsStatic = [
#     'Total Energy (Kilowatt-hr)', 'Average Energy (Kilowatt-hr)', 'Interval Energy (Kilowatt-hr)',
#     'Average Interval Energy (Kilowatt-hr)', 'Number of completed tasks', 'Number of completed tasks per interval',
#     'Average Response Time (seconds)', 'Total Response Time (seconds)', 'Average Migration Time (seconds)',
#     'Total Migration Time (seconds)', 'Number of Task migrations', 'Average Wait Time (intervals)',
#     'Average Wait Time (intervals) per application', 'Average Completion Time (seconds)',
#     'Total Completion Time (seconds)', 'Average Response Time (seconds) per application',
#     'Cost per container (US Dollars)', 'Fraction of total SLA Violations', 'Fraction of SLA Violations per application',
#     'Interval Allocation Time (seconds)', 'Number of completed tasks per application', "Fairness (Jain's index)",
#     'Fairness', 'Fairness per application', 'Average CPU Utilization (%)', 'Average number of containers per Interval',
#     'Average RAM Utilization (%)', 'Scheduling Time (seconds)', 'Average Execution Time (seconds)'
# ]
#
# yLabelStatic2 = {
#     'Average Completion Time (seconds)': 'Number of completed tasks'
...
#
# yLabelsTime = [
#     'Interval Energy (Kilowatts)', 'Number of completed tasks', 'Interval Response Time (seconds)',
#     'Interval Migration Time (seconds)', 'Interval Completion Time (seconds)', 'Interval Cost (US Dollar)',
...
#
# # 定义主分析函数
# def analyze_and_plot_results(env):
#     if not isinstance(env, str):
#         # 使用类名作为环境标识符
#         env = env.__class__.__name__  # 获取类的名称，例如 "Simulator"
#
#     # 设置路径
#     path = 'all_datasets/' + env + '/'
#     save_path = 'results/' + env + '/'
...
#     all_stats = dict(zip(load_models, all_stats_list))
#
#     if all_stats.get(sla_baseline) is None:
#         raise ValueError(f"Model {sla_baseline} data not found or failed to load.")
#
#     # 计算 SLA
#     if env == 'framework':
#         sla = {}
...
#     # 绘制柱状图
#     for ylabel in yLabelsStatic:
#         if Models[0] not in Data[ylabel]:
#             continue
#         plt.figure(figsize=size)
#         plt.xlabel('Model')
#         plt.ylabel(ylabel.replace('%', '\%').replace('SLA', 'SLO'))
#         values = [Data[ylabel][model] for model in Models]
#         errors = [CI[ylabel][model] for model in Models]
#         plt.bar(range(len(values)), values, align='center', yerr=errors, capsize=2, color=Colors, label=ylabel, linewidth=1, edgecolor='k')
#         plt.xticks(range(len(values)), Models, rotation=rot)
#         # plt.savefig(save_path + 'Bar-' + ylabel.replace(' ', '_') + ".pdf")
#
#         ## 在保存图片之前，确保路径存在，并打印保存的路径
#         os.makedirs(save_path, exist_ok=True)  # 确保目录存在
#         print("保存图片aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
#         image_filename = os.path.join(save_path, f"Bar_{ylabel.replace(' ', '_')}.pdf")
...
#         if Models[0] not in Data[ylabel]:
#             continue
#         table[ylabel] = [fstr(Data[ylabel][model]) + '+-' + fstr(CI[ylabel][model]) for model in Models]
#     df = pd.DataFrame(table)
#     df.to_csv(save_path + 'table.csv')