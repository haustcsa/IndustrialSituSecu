...
# 在训练过程中收集并可视化各种模型的评估指标和结果,包括损失函数、准确度、预测分数等，以及一些更复杂的可视化，如 t-SNE、热图等
...
def smoother(y, box_pts=1):
    ...
    box = np.ones(box_pts)/box_pts  # box_pts是滑动窗口大小
    y_smooth = np.convolve(y, box, mode='same') # mode='same' 保证输出与输入数据 y 的长度相同
    return y_smooth
...
class Model_Plotter():
	def __init__(self, env, modelname):
		...
		for f in glob.glob(self.folder + '/*'): os.remove(f)  # 在每次训练开始时，Model_Plotter 会清理图形文件夹中的所有文件
	...
	def update_class(self, prototypes, correct):
		self.protoypes.extend(prototypes)
	...
	def plot(self, accuracy_list, epoch):
		...
		self.plot_heatmap('Anomaly Scores', 'Prediction', 'Ground Truth', self.source_anomaly_scores, self.target_anomaly_scores) # plot_heatmap是异常分数热图
	...
	def plot1(self, name1, data1, smooth = True, xlabel='Epoch'): # 绘制一条线图
	...
	def plot2(self, name1, name2, data1, data2, smooth = True, xlabel='Epoch'): # 绘制两个y轴共享的折线图
	...
	def plot_heatmap(self, title, name1, name2, data1, data2):  # 绘制热图
...
class GAN_Plotter():
	def __init__(self, env, gname, dname, training = True):
		...
		self.smooth_ns = 0  ####################### 用于存储平滑后的 ns
		self.smooth_os = 0  ###################### 用于存储平滑后的 os
		self.alpha = 0.9  ######################## 平滑系数，可以调整
		...
		# # 加载字体文件
		# font_path = "C:/Users/Lenovo/AppData/Local/Microsoft/Windows/Fonts/MavenPro-Regular.ttf"  #####
	...
	def plot_test(self, hosts_from):
		...
		print(f"Epoch: {self.epoch}, new_score_better: {self.new_score_better}")  #################
	...
	def plot(self, accuracy_list, epoch, ns, os):
		...
		######################### 更新平滑后的 ns 和 os
		self.smooth_ns = self.alpha * ns + (1 - self.alpha) * self.smooth_ns
		self.smooth_os = self.alpha * os + (1 - self.alpha) * self.smooth_os
		####################### 计算 Improvement Ratio
		improvement = (self.smooth_ns > self.smooth_os)  # 如果平滑后的 ns 大于平滑后的 os
		self.improvement_ratio.append(improvement)
		########################### 计算 Improvement Ratio 的平均值
		improvement_ratio_value = sum(self.improvement_ratio) / len(
			self.improvement_ratio) if self.improvement_ratio else 0
		# 打印 Improvement Ratio
		print(f"Improvement Ratio: {improvement_ratio_value:.4f}")
