...
# def form_test_dataset(data):  # 该函数的目的是根据给定的数据，按 3 个维度一组，检测每个组内是否存在异常（通过百分位数判断）。如果某个组内有异常，返回该组异常值的维度索引（即是 0, 1, 或 2）。
...
# 	return anomaly_any_dim + 0, anomaly_which_dim   # `anomaly_any_dim`：每组维度中是否有异常。`anomaly_which_dim`：每组维度中异常发生在哪个维度。
...
def load_dataset(folder, model):
	time_data = load_npyfile(folder, data_filename)
	print('dadadadada',data_filename)
	time_data = normalize_time_data(time_data) # Normalize data
	train_schedule_data = torch.tensor(load_npyfile(folder, schedule_filename)).double()
	train_time_data = convert_to_windows(time_data, model)
...
def save_model(folder, fname, model, optimizer, epoch, accuracy_list):
	path = os.path.join(folder, fname)
	print(f"Saving model to: {path}")  ####### 打印保存路径，便于调试或日志记录
...
def load_model(folder, fname, modelname):
	import recovery.SGATGANSrc.src.models
	path = 'D:/SGATGAN-master-implement/recovery/SGATGANSrc/checkpoints/simulator_FPE_16.ckpt'
	print('pathuuuuu',path)
	#print('pathuuuuu1', folder,modelname)
	model_class = getattr(recovery.SGATGANSrc.src.models, modelname)
	model = model_class().double()
	optimizer = torch.optim.AdamW(model.parameters() , lr=model.lr, weight_decay=1e-5)
	if os.path.exists(path):
		print(f"{color.GREEN}Loading pre-trained model1: {model.name}{color.ENDC}")
		checkpoint = torch.load(path)
		model.load_state_dict(checkpoint['model_state_dict'])
		model.prototype = checkpoint['model_prototypes']
		print('prototype333333', model.prototype)
