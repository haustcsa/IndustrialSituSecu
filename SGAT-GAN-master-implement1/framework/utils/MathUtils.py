...
import scipy
import warnings
# 这个代码实现了局部加权回归（Locally Weighted Regression，简称 LOESS），并提供了对回归结果的健壮性增强（robustification）。
...
def loess(xvals, yvals, alpha, poly_degree=1, robustify=False):
    ...
    # 健壮性增强：如果 robustify 参数为 True，会执行一个迭代过程，使用加权的残差对回归结果进行健壮性增强。每次迭代中计算新的残差并调整回归系数，直到估计值变化不大或达到最大迭代次数。
    if robustify==True:
